...
class LazyLLMDataset(Dataset):
    def __init__(self, dataset: HfDataset, template: Template, *, VQA_features = None, try_fetch_time: int = 20) -> None:
        self.dataset = dataset
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        VQA_features_item = self.VQA_features[idx]
        if res is not None and len(res[0]) > 0:
            data, _ = res
            data['img_feat'] = VQA_features_item["img_feat"].unsqueeze(0)
            data['ques_ids'] = VQA_features_item["ques_ids"].unsqueeze(0)
            data['ans_vec'] = VQA_features_item["ans_vec"].unsqueeze(0)
            return data
...
def _prepare_inputs(model: PreTrainedModel,
    ...
    inputs['img_feat'] = VQA_features['img_feat'].unsqueeze(0)
    inputs['ques_ids'] = VQA_features['ques_ids'].unsqueeze(0)
    inputs['ans_vec'] = VQA_features['ans_vec'].unsqueeze(0)
