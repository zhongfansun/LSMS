...
def get_model_tokenizer(model_type: str,
    ...
    szfnote:
        args.model_type:      llava1_5-13b-instruct
        args.torch_dtype:     torch.bfloat16
        model_kwargs:         {'low_cpu_mem_usage': True, 'device_map': 'cuda:0'}
        model_id_or_path=args.model_id_or_path:    'swift/llava-1.5-13b-hf'
        revision=args.model_revision:              'master'
        kwargs:               {'is_training': True, 'max_length': 2048, 'use_unsloth': False, 'load_in_4bit': False}
    """
    model_dir = kwargs.pop('model_dir', None)  # compat with swift<1.7
    ...
    model_info = MODEL_MAPPING[model_type]
    '''
    {'model_id_or_path': 'swift/llava-1.5-13b-hf',
     'lora_target_modules': '^(language_model|multi_modal_projector)(?!.*(lm_head|output|emb|wte|shared)).*',
     'template': 'llava1_5', 'requires': ['transformers>=4.36'], 'torch_dtype': None, 'ignore_file_pattern': None,
     'hf_model_id': 'llava-hf/llava-1.5-13b-hf', 'revision': 'master', 'eos_token': '</s>', 'support_flash_attn': True,
     'support_vllm': True, 'support_lmdeploy': True, 'tags': ['multi-modal', 'vision'], 'get_function': < function
    get_model_tokenizer_llava_1_5 at 0x7fabb5d600d0 >}
    '''
