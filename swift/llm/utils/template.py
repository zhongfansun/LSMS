...
class Template:
    ...
    def data_collator(self, batch: List[Dict[str, Any]], padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        img_feat = [b['img_feat'] for b in batch if b.get('img_feat') is not None]
        if len(img_feat) > 0:
            res['img_feat'] = torch.concat(img_feat)

        ques_ids = [b['ques_ids'] for b in batch if b.get('ques_ids') is not None]
        if len(ques_ids) > 0:
            res['ques_ids'] = torch.concat(ques_ids)

        ans_vec = [b['ans_vec'] for b in batch if b.get('ans_vec') is not None]
        if len(ans_vec) > 0:
            res['ans_vec'] = torch.concat(ans_vec)
