...
def prepare_model_template(args: InferArguments,
    ...
    ckpt = torch.load("data_files/mcan_ft_okvqa.pkl")
    model.mcan_net.load_state_dict(ckpt['state_dict'], strict=True)
...
def llm_infer(args: InferArguments) -> Dict[str, List[Dict[str, Any]]]:
    ...
    if args.eval_human:
        ...
        if args.infer_backend in {'vllm', 'lmdeploy'} and not args.stream:
            ...
            for i_features, data in enumerate(val_dataset):
                VQA_features = val_VQA_features[i_features]
                ...
                if args.infer_backend in {'vllm', 'lmdeploy'}:
                    ...
                    response, _ = inference(
                        model, template, VQA_features, stream=args.stream and args.verbose, verbose=args.verbose, **kwargs)
