...
def llm_sft(args: SftArguments) -> Dict[str, Any]:
    ...
    ckpt = torch.load("data_files/mcan_ft_okvqa.pkl") #, map_location='cpu'
    model.mcan_net.load_state_dict(ckpt['state_dict'], strict=True) #, , , assign=True
    ...
    if args.packing:
        ...
        train_VQA_features = torch.load('data_files/okvqa_train_data.pt')
        val_VQA_features = torch.load('data_files/okvqa_val_data.pt')

        train_dataset = LazyLLMDataset(train_dataset, template, VQA_features=train_VQA_features)
        if val_dataset is not None:
            val_dataset = LazyLLMDataset(val_dataset, template, VQA_features=val_VQA_features)
