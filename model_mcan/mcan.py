...
class MCAN(nn.Module):
    ...
    def __init__(self, __C, answer_size):
        ...
        config = AutoConfig.from_pretrained(__C.BERT_VERSION)
    ...
    def forward(self, input_tuple, output_answer_latent=False):
        ...
        lang_feat1 = self.attflat_lang(
            lang_feat,
            lang_feat_mask
        )
        img_feat1 = self.attflat_img(
            img_feat,
            img_feat_mask
        )

        proj_feat = lang_feat1 + img_feat1
        answer_latent = self.proj_norm(proj_feat)
        proj_feat = self.proj(answer_latent)

        if output_answer_latent:
            return proj_feat, answer_latent, lang_feat, img_feat
